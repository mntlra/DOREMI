...
def get_thresh_preds(probs, gt):
                # (cand: list, num_gt: int, correct: int, num_pred: int):
    '''
    select threshold for relation predictions and return the predictions.
    Input:
        :probs: list of relation probabilities
        :gt: list of ground_truth labels.
    Output:
        :preds: 0-1 matrix to select predictions.
    ...
    probs = probs.flatten()
    # argsort returns indices sorted in ascending, since we want it in descending order we negate the probability.
    ...
    for idx in tqdm(sorted_indices, desc="probs"):
        correct += gt_flat[idx]
        num_pred += 1
    ...
    thresh = probs[sorted_indices[f1_pos]]

    # return predictions
    preds_indices = sorted_indices[:f1_pos+1]
    preds = np.zeros(probs.shape)
    preds[preds_indices] = 1
    preds = preds.reshape(ori_shape)

    return preds, thresh

def get_preds(probs, threshold):
    '''
    Return relation predictions.
    Input:
        :probs: list of relation probabilities
        :threshold: threshold for relation predictions.
    Output:
        :preds: 0-1 matrix to select predictions.
    '''
...
def to_official(data_dir, preds: list, probs: list, features: list):
    '''
    ...
    rel2id = json.load(open(data_dir + '/meta/rel2id.json', 'r'))
    ...
    for i in tqdm(range(preds.shape[0]), desc="preds"): # for each entity pair
        ...
        for ix, p in enumerate(pred): # for each predicted relation label (topk)
            curr_result = {
                    'title': title[i],
                    'h_idx': h_idx[i],
                    't_idx': t_idx[i],
                    'r': id2rel[p],
                    'score': float(format(probs[i,p], '.7g'))
                }
...
def official_evaluate(logger, tmp, path, train_file="train_annotated.json", dev_file="dev.json"):
    '''
    ...
    # logger.logger.info(f"Path: {path}")
