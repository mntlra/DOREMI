...
class EviConfig(object):
	...
	def get_N2_train_batch(self):
		...
		relation_label = torch.LongTensor(self.batch_size).to(self.device)
		evidence_label = torch.Tensor(self.batch_size, self.sent_limit).to(self.device)
		sent_mask = torch.Tensor(self.batch_size, self.sent_limit).to(self.device)

		sent_h_mapping = torch.Tensor(self.batch_size, self.sent_limit, self.max_length).to(self.device)
		sent_t_mapping = torch.Tensor(self.batch_size, self.sent_limit, self.max_length).to(self.device)
