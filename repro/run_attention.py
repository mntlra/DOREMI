...
def load_input(batch, device, tag="dev", compute_attention=False, teacher=None):
    ...
    input = {'input_ids': batch[0].to(device),
            ...
            'hts': batch[4],
            'sent_pos': batch[5],
            'sent_labels': batch[6].to(device) if (not batch[6] is None) and (compute_attention is False) else None,
            'teacher_attns': None,
            'tag': tag
            }

    if compute_attention:
        batch_attention_matrix = compute_attention_matrix(teacher, batch)
        # Padding attention matrices -- code from utils.fn_collate
        max_len = max([len(input_ids) for input_ids in batch[0]])
        # USING functional pad
        bam_numpy = [a.cpu().numpy() for a in batch_attention_matrix]
        attns = [np.pad(attn, ((0, 0), (0, max_len - attn.shape[1]))) for attn in bam_numpy]
        attns = torch.from_numpy(np.concatenate(attns, axis=0))
        input['teacher_attns'] = attns.to(device)

    return input


def compute_attention_matrix(teacher, batch):
    ...
    inputs = load_input(batch, teacher["device"], tag)
...
def train(args, model, train_features, dev_features, Logger, compute_attention=False, teacher=None):
    """
        :param compute_attention: (bool) whether to compute the attention matrix for the document.
        :param teacher: (dict) dictionary containing the teacher model to use for computing the attention matrix
                        (key: model) and the device where the teacher model is stored (key: device).
    """

    def finetune(features, optimizer, num_epoch, num_steps, Logger, compute_attention=False, teacher=None):
        best_score = -1
        ...
        scaler = GradScaler()
        Logger.logger.info("Total steps: {}".format(total_steps))
        Logger.logger.info("Warmup steps: {}".format(warmup_steps))
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if compute_attention:
                    inputs = load_input(batch, args.device,
                                        compute_attention=compute_attention, teacher=teacher)
                else:
                    inputs = load_input(batch, args.device)
                ...
                if (step + 1) == len(train_dataloader) or (args.evaluation_steps > 0 and num_steps % args.evaluation_steps == 0 and step % args.gradient_accumulation_steps == 0):
                    ...
                    dev_scores, dev_output, official_results, results = evaluate(args, model, dev_features, Logger,
                                                                                 tag="dev")
                    # wandb.log(dev_scores, step=num_steps)
                    
                    Logger.logger.info(dev_output)
                    if dev_scores["dev_F1_ign"] > best_score:
                        ...
                        Logger.logger.info(f"saving model checkpoint into {ckpt_file} ...")
                    ...
                    if epoch == train_iterator[-1]: # last epoch
                        ...
                        dump_to_file(best_offi_results, pred_file, best_output, score_file, Logger,
                                     results=best_results, res_path=results_file)
    ...
    set_seed(args)
    model.zero_grad()
    finetune(train_features, optimizer, args.num_train_epochs, num_steps, Logger, compute_attention, teacher)

def evaluate(args, model, features, Logger, tag="dev"):
    ...
    if args.save_attn:
        ...
        Logger.logger.info(f"saving attentions into {attns_path} ...")
        with open(attns_path, "wb") as f:
            pickle.dump(attns, f)

    return scores, output, official_results, results

def dump_to_file(offi:list, offi_path: str, scores: list, score_path: str, Logger, results: list = [], res_path: str = "", thresh: float = None):
    '''
    ...
    if len(results) != 0:
        assert res_path != ""
        Logger.logger.info(f"saving topk results into {res_path} ...")
        json.dump(results, open(res_path, "w"))
    
    if thresh != None:
        thresh_path = os.path.join(os.path.dirname(offi_path), "thresh")
        if not os.path.exists(thresh_path):
            Logger.logger.info(f"saving threshold into {thresh_path} ...")
...
def main():
    ...
    # --- TO ENABLE wandb again, uncomment this and all mentions in the code #
    # ----------------------
    # --- -------- ----- ---
    # ----------------------
    ...
    logger_name = args.save_path + "/" + time + ".log"
    my_logger = Logger(logger_name)

    if not torch.cuda.is_available():
        raise ValueError(f"Cuda is not available: cuda.is_available: {torch.cuda.is_available()};"
                         f" Number of devices available: {torch.cuda.device_count()}")
    if torch.cuda.device_count() < 2:
        raise ValueError(f"Number of devices available should be at least two, but the number of devices available is"
    ...
    device = torch.device(torch.cuda.device_count()-2)
    my_logger.logger.info(f"*** Student Model will be loaded to device: {device} ***")
    ...
    if args.do_train:  # Training
        ...
        train_features = read(train_file, tokenizer, my_logger, transformer_type=args.transformer_type,
                              max_seq_length=args.max_seq_length)
        dev_features = read(dev_file, tokenizer, my_logger, transformer_type=args.transformer_type,
                            max_seq_length=args.max_seq_length)

        if args.teacher_sig_path != "":
            # Load teacher model
            ...
            teacher_config.transformer_type = args.transformer_type

            set_seed(args)

            teacher_config.cls_token_id = teacher_tokenizer.cls_token_id
            teacher_config.sep_token_id = teacher_tokenizer.sep_token_id

            teacher_model = DocREModel(teacher_config, teacher_model, teacher_tokenizer,
                               num_labels=args.num_labels,
                               max_sent_num=args.max_sent_num,
                               evi_thresh=args.evi_thresh)

            # where to store the teacher model
            teacher_device = torch.device(torch.cuda.device_count()-1)
            teacher_model.to(teacher_device)
            my_logger.logger.info(f"*** Teacher Model will be loaded to device: {teacher_device} ***")

            model_path = os.path.join(args.teacher_sig_path, "best.ckpt")
            teacher_model.load_state_dict(torch.load(model_path), strict=False)
            teacher = {"model": teacher_model, "device": teacher_device}
            # compute attention matrix using teacher model
            train(args, model, train_features, dev_features, my_logger, compute_attention=True, teacher=teacher)
        else:
            # Standard training
            train(args, model, train_features, dev_features, my_logger)
        ...
        test_features = read(test_file, tokenizer, my_logger, transformer_type=args.transformer_type, max_seq_length=args.max_seq_length)
        
        if args.eval_mode != "fushion":

            test_scores, test_output, official_results, results = evaluate(args, model, test_features, my_logger, tag="test")
            ...
            dump_to_file(official_results, offi_path, test_output, score_path, my_logger, results=results, res_path=res_path)
            ...
            pseudo_test_features = read(test_file, tokenizer, my_logger, max_seq_length=args.max_seq_length,
                                        single_results=results)
        
            pseudo_test_scores, pseudo_output, pseudo_official_results, pseudo_results = evaluate(args, model, pseudo_test_features, my_logger, tag="test")

            if 'thresh' in os.listdir(args.load_path):
                with open(os.path.join(args.load_path, "thresh")) as f:
                    thresh = json.load(f)
                my_logger.logger.info(f"Threshold loaded from file: {thresh}")
            else:
                thresh = None
            
            merged_offi, thresh = merge_results(results, pseudo_results, test_features, my_logger, thresh)
            ...
            dump_to_file(merged_offi, offi_path, merged_output, score_path, my_logger, thresh=thresh)
